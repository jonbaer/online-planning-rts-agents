...
for res in vol_root:
	...
	tVol = volumetric.Volumetric(tName, tRate, tInitAmt)
...
for res in gUnary_root:
	...
	tMinCost = float(res.get('minCost'))
	tGasCost = float(res.get('gasCost'))
	tSuppCost = float(res.get('suppCost'))
	...
	#tExist = int(res.get('exist'))
	#print(tUnary.name)
	tUnaryGoal = unary.Unary(tName,tExist,tMinCost,tGasCost,tSuppCost,tDurAmt)
	#Adds a unary object to the initial game state if specified by setting 'exist = Number of instances of resource' inside unary_resource.xml
	#for i in range(1,tExist+1):
	#	print i
	#	tCopy = unary.Unary(tName)
...
for unit in gUnit_root:
	...
	tMinCost = float(unit.get('minCost'))
	tGasCost = float(unit.get('gasCost'))
	tSuppCost = float(unit.get('suppCost'))
	tDurAmt = float(unit.get('durAmt'))
	tUnit = combat_unit.CombatUnit(tName, tExist,tMinCost,tGasCost,tSuppCost,tDurAmt)
